...
class EmailHandler:
    ...
    def send_to_admin(name: str, email: str):  # Sends an email to the admin with JWT secure encoded links to approve or reject the user registration.
        ...
        approval_url = f"http://127.0.0.1:8000/approve_user?token={approve_token}"
        rejection_url = f"http://127.0.0.1:8000/reject_user?token={reject_token}"
